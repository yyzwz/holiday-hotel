            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    <Form-item label="" prop="content">
                        <Input type="text" v-model="searchForm.content" placeholder="请输入评论内容" clearable style="width: 200px" />
                    </Form-item>
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            this.loading = true;
            getDormitoryDiscussList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
